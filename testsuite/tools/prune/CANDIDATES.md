...
(~1200s / ~20min on fedrawhide). Subsumed by `buildok`/`semok`,
...
stress), `div0` (must not crash on /0), `floatingpoint`, `atomic`,
`addr_op`, `abort`, `exit`, `beginenderror`, `timers`, `argv`, bug
regressions, SDT, etc.
